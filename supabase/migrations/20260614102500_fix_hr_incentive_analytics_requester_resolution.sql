...
begin
  ...
  return (
    ...
    driver_contract_amounts as (
      select
        coalesce(nullif(trim(fr.requester_name), ''), 'Sin solicitante') as driver_name,
      ...
      from filtered_requests fr
      group by
        coalesce(nullif(trim(fr.requester_name), ''), 'Sin solicitante'),
    ...
    amount_by_driver as (
      select
        dca.driver_name,
        coalesce(sum(dca.amount), 0)::numeric(14,2) as total_amount
      from driver_contract_amounts dca
      group by dca.driver_name
      order by total_amount desc, dca.driver_name asc
      limit 12
    ),
    contract_options as (
      select distinct
    ...
    select jsonb_build_object(
      ...
      coalesce((
        select jsonb_agg(
          jsonb_build_object(
            'driver_name', abd.driver_name,
            'total_amount', abd.total_amount,
            'contracts',
            coalesce((
              select jsonb_agg(
                jsonb_build_object(
                  'contract_code', dca.contract_code,
                  'contract_label', dca.contract_label,
                  'amount', dca.amount
                )
                order by dca.amount desc, dca.contract_label asc
              )
              from driver_contract_amounts dca
              where dca.driver_name = abd.driver_name
            ), '[]'::jsonb)
          )
          order by abd.total_amount desc, abd.driver_name asc
        )
        from amount_by_driver abd
      ), '[]'::jsonb),
      'filter_options',
      jsonb_build_object(
        'contracts',

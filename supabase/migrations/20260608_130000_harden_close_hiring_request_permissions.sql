...
create or replace function public.user_can_close_hiring_request(
  target_user_id uuid,
...
begin
  ...
  if public.user_is_admin(target_user_id) or public.user_has_role(target_user_id, 'reclutamiento') then
    return true;
  end if;

  select hr.cost_center_code
    into request_cost_center_code
    from public.hiring_requests hr
   where hr.id = target_request_id;

  if request_cost_center_code is null then
    return false;
  end if;
...
revoke all on function public.user_can_close_hiring_request(uuid, uuid) from public, anon;
grant execute on function public.user_can_close_hiring_request(uuid, uuid) to authenticated;

drop function if exists public.close_hiring_request(uuid, text);

...
-- rollback: forward-only; restore the prior assignment matching contract in a subsequent audited migration.
-- purpose: a roster assignment is operational only while its stored BUK contract
-- and area match the current employee projection; historical assignments remain.
begin;
...
immutable
as $$
  select lower(trim(coalesce(p_assignment_contract_code, ''))) = lower(trim(coalesce(p_current_contract_code, '')))
     and lower(trim(coalesce(p_assignment_area_name, ''))) = lower(trim(coalesce(p_current_area_name, '')));
$$;
      ...
      with active_workers as (
        ...
        left join lateral (
          ...
          where wr.employee_buk_employee_id = fw.buk_employee_id
            and public.hr_roster_assignment_matches_current_buk(wr.contract_code, wr.area_name, fw.contract_code, fw.area_name)

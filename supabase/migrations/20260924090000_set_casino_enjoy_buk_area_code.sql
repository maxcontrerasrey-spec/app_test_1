...
-- Completa la ruta BUK entregada por Operaciones para permitir la generación efectiva.
  ...
  if not exists (
    select 1
     ...
     where bcm.id = mapping_record.id
       ...
       and bcm.buk_area_code = '9959890001:0001'
       and bcm.is_operational = true
       and bcm.is_one_to_one = true
  ) then
    raise exception 'El mapping BUK de CASINO ENJOY no quedó listo para generación';
  end if;
end;
$$;

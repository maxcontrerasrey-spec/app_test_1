...
begin
    ...
    from (
      select
        ...
        case rcc.stage_code
          ...
          when 'medical_exam' then 5
          when 'documentation' then 6
          when 'rejected' then 7
          when 'withdrawn' then 8
          when 'ready_to_hire' then 9
          when 'hired' then 10
          else 11
        end as sort_stage_rank,
        coalesce(rcc.created_at, rcc.stage_entered_at, rc.created_at) as sort_candidate_created_at
      ...
      left join public.profiles owner_profile
        on owner_profile.id = owner_assignment.user_id
      left join lateral (
        select count(distinct rcc_active.recruitment_case_id)::integer as active_process_count
        ...
        where rcc_active.candidate_profile_id = rcc.candidate_profile_id
          ...
          and rc_active.status not in ('cancelled', 'closed_unfilled')
      ...
      where public.user_can_view_recruitment_process_summary(
        current_user_id,
        rc.id
      )
      and (
  ...
  if public.user_has_role(current_user_id, 'control_contratos')
    or public.user_has_role(current_user_id, 'reclutamiento')
    or public.user_is_admin(current_user_id)
  then
    select coalesce(
      jsonb_agg(personnel_row.payload order by personnel_row.sort_ready_at asc),
      '[]'::jsonb
    )
    ...
    from (
      select
        jsonb_build_object(
          'recruitment_case_candidate_id', rcc.id,
          'recruitment_case_id', rc.id,
          'candidate_profile_id', cp.id,
          'case_code', rc.case_code,
          'folio', hr.folio,
          'candidate_name', cp.full_name,
          'candidate_national_id', cp.national_id,
          ...
          'cost_center_code', rc.cost_center_code,
          'cost_center_name', rc.cost_center_name,
          'ready_for_contract_at', coalesce(rcc.documentation_completed_at, rcc.updated_at),
          'owner_name', owner_profile.full_name,
          'candidate_status', rcc.stage_code,
          'driver_license_number', cp.driver_license_number,
          'driver_license_class', cp.driver_license_class,
          'driver_license_expiry', cp.driver_license_expiry,
          'candidate_number', cp.candidate_number
        ) as payload,
        coalesce(rcc.documentation_completed_at, rcc.updated_at) as sort_ready_at
      from public.recruitment_case_candidates rcc
      join public.recruitment_cases rc
        on rc.id = rcc.recruitment_case_id
      join public.hiring_requests hr
      ...
      left join public.profiles owner_profile
        on owner_profile.id = owner_assignment.user_id
      where rcc.stage_code = 'ready_to_hire'

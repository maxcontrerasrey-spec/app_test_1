...
begin
  if tg_op = 'UPDATE'
     ...
     and new.company_entry_date is not distinct from old.company_entry_date
     and new.private_role is not distinct from old.private_role
     and new.payment_method is not distinct from old.payment_method
     and new.payment_period is not distinct from old.payment_period
     and new.pension_regime is not distinct from old.pension_regime
     and new.increase_quote_one_percent is not distinct from old.increase_quote_one_percent
     and new.health_provider is not distinct from old.health_provider
     and new.afc_regime is not distinct from old.afc_regime
     and new.retirement_regime is not distinct from old.retirement_regime then
    return null;
  end if;

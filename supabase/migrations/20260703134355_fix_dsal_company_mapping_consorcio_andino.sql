...
set
  company_name = 'Consorcio Andino SPA',
  updated_at = timezone('utc', now())
where (
    bcm.contract_number = '6170400011:0001'
    or bcm.buk_area_name_normalized = public.normalize_buk_area_name('CODELCO - DSAL')
  )
  and bcm.company_name is distinct from 'Consorcio Andino SPA';

update public.internal_mobility_requests imr
set
  destination_company_name = 'Consorcio Andino SPA',
  requires_termination = (
    imr.current_company_name is not null
    and imr.current_company_name is distinct from 'Consorcio Andino SPA'
  ),
  updated_at = timezone('utc', now())
where (
    imr.destination_contract_id in (
      ...
      where c.contract_number = '6170400011:0001'
    )
    or public.normalize_buk_area_name(imr.destination_area_name) = public.normalize_buk_area_name('CODELCO - DSAL')
  )
  and (
    imr.destination_company_name is distinct from 'Consorcio Andino SPA'
...
set payload = jsonb_set(
  jsonb_set(
    ...
    to_jsonb('Consorcio Andino SPA'::text),
    true
  ),
  '{requires_termination}',
  to_jsonb(
...
where imr.id = imrs.internal_mobility_request_id
  ...
  and (
    imrs.payload ->> 'destination_company_name' is distinct from 'Consorcio Andino SPA'

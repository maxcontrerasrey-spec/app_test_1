...
-- Actualizamos el RPC get_recruitment_control_dashboard_v2 para incluir folios cerrados o cancelados en active_cases
-- Esto es necesario porque el frontend usa esta misma lista ("activeCases") para todas las pestañas de filtrado ("Cubiertos", "Rechazados / Cerrados", etc.)
-- Si los filtramos en la BD, el frontend nunca podrá mostrarlos en esas pestañas.
...
begin
  ...
  from (
    ...
    limit 50 -- Limite para no ahogar la vista si hay cientos de cerrados
  ) as case_row;
  ...
  return jsonb_build_object(
    ...
    'candidateControl', candidate_control,
    'personnelToHire', personnel_to_hire
  );
end;
$function$;

...
begin
  ...
  return (
    ...
    assigned_ceco_scope as (
      select distinct regexp_replace(trim(cca.cost_center_code), '\.0+$', '', 'g') as normalized_ceco
    ...
    workforce_ceco_scope as (
      select distinct regexp_replace(trim(coalesce(e.contract_code, '')), '\.0+$', '', 'g') as normalized_ceco
      ...
      select distinct regexp_replace(trim(vhs.cost_center_code), '\.0+$', '', 'g') as normalized_ceco
    ...
    incentives_summary as (
      select
        ...
        coalesce(sum(hir.calculated_amount) filter (where hir.status <> 'R'), 0)::numeric(14,2) as total_amount

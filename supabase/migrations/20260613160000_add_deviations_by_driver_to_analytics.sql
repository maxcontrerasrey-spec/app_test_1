...
begin
  normalized_status := coalesce(trim(upper(p_status)), 'A');
  if normalized_status not in ('A', 'P', 'E', 'R', 'F', 'C') then
    normalized_status := 'A';
  end if;
  ...
  return (
    ...
    deviations_by_driver as (
      select
        fr.requester_name as driver_name,
        count(*) filter (where fr.is_out_of_deadline)::int as out_of_deadline_count,
        count(*) filter (where fr.is_contract_mismatch)::int as contract_mismatch_count,
        (
          count(*) filter (where fr.is_out_of_deadline)
      ...
      from filtered_requests fr
      group by fr.requester_name
      having (
        count(*) filter (where fr.is_out_of_deadline)
        + count(*) filter (where fr.is_contract_mismatch)
      ) > 0
      order by total_deviations desc, fr.requester_name asc
      limit 12
    ),
    contract_options as (
      select distinct
    ...
    select jsonb_build_object(
      ...
      'deviations_by_driver',
      coalesce((
        select jsonb_agg(
          jsonb_build_object(
            'driver_name', dbd.driver_name,
            'out_of_deadline_count', dbd.out_of_deadline_count,
            'contract_mismatch_count', dbd.contract_mismatch_count,
            'total_deviations', dbd.total_deviations
          )
        )
        from deviations_by_driver dbd
      ), '[]'::jsonb),
      'filter_options',
      jsonb_build_object(
        'contracts',

...
-- The initial queue migration was deployable but its PL/pgSQL body was not
-- exercised during migration: it referenced the output variable as a column
-- and inserted a null source_document_id. Keep the correction forward-only.
      ...
      from jsonb_array_elements(
        case
          when jsonb_typeof(coalesce(p_existing_documents, '[]'::jsonb)) = 'array'
            and jsonb_array_length(coalesce(p_existing_documents, '[]'::jsonb)) > 0

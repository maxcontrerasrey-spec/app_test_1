alter table public.hr_incentive_rate_rules
  add column if not exists union_name text null;

alter table public.hr_incentive_requests
  add column if not exists employee_union_name text null;

create or replace function public.get_hr_incentive_union_name(
  p_raw_payload jsonb
)
returns text
language sql
immutable
as $function$
  select nullif(trim(coalesce(p_raw_payload -> 'current_job' ->> 'union', '')), '');
$function$;

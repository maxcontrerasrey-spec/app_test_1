...
create or replace function public.find_buk_employee_status_by_rut(
  p_national_id text
)
returns table (
  match_found boolean,
...
declare
  ...
  normalized_input text := upper(regexp_replace(trim(coalesce(p_national_id, '')), '[^0-9A-Za-z]', '', 'g'));
begin
  ...
  with ranked_employees as (
    select
      ...
      e.is_active,
      coalesce(
        nullif(trim(e.raw_payload ->> 'active_until'), ''),
        nullif(trim(e.raw_payload #>> '{current_job,end_date}'), ''),
        nullif(trim(e.raw_payload #>> '{current_job,active_until}'), ''),
        nullif(trim(e.raw_payload ->> 'termination_date'), ''),
        nullif(trim(e.raw_payload ->> 'end_date'), '')
      ) as exit_date_raw,
      row_number() over (
        order by
  ...
  select
    true as match_found,
    ranked_employees.full_name as name,
    ranked_employees.status,
    ranked_employees.is_active,
    case
      when ranked_employees.exit_date_raw ~ '^\d{4}-\d{2}-\d{2}$' then ranked_employees.exit_date_raw::date
      else null
    end as exit_date
  from ranked_employees
  where ranked_employees.rn = 1;
end;
$function$;

revoke all on function public.find_buk_employee_status_by_rut(text) from public, anon;
grant execute on function public.find_buk_employee_status_by_rut(text) to authenticated;
  ...
  if can_access_candidate_control then
    select coalesce(
      jsonb_agg(
        ...
        order by candidate_row.sort_stage_entered_at desc, candidate_row.sort_created_at desc
      ),
      '[]'::jsonb
    )
      ...
      left join lateral (
        select count(*) as active_process_count
         ...
         where sibling_rcc.candidate_profile_id = cp.id
           ...
           and (
             sibling_rc.status not in ('filled', 'closed_unfilled', 'cancelled')
             or (
               sibling_rc.status = 'cancelled'
               and exists (
                 select 1
                   from public.hiring_requests sibling_hr
                  where sibling_hr.id = sibling_rc.hiring_request_id
                    and sibling_hr.status = 'closed'
               )
             )
           )
    ...
    select coalesce(
      jsonb_agg(
        person_row.payload
        order by person_row.sort_hired_at desc, person_row.sort_created_at desc
      ),
      '[]'::jsonb
    )
    ...
    from (
      ...
      where public.user_can_access_recruitment_case(current_user_id, rc.id)
        and (
          rc.status not in ('filled', 'closed_unfilled', 'cancelled')
          or (rc.status = 'cancelled' and hr.status = 'closed')
        )
        and rcc.stage_code = 'hired'

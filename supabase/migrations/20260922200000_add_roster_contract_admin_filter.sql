...
-- rollback: forward-only; restore the previous catalog/calendar contract in a later audited migration.
-- scope: add an operational contract-administrator scope without changing existing permissions or assignments.
begin;
...
begin
  ...
  return jsonb_build_object(
    ...
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'value', admins.administrator_name,
          'label', admins.administrator_name
        )
        order by admins.administrator_name
      )
      from (
        select min(trim(bcm.contract_admin_name)) as administrator_name
        ...
        where bcm.is_operational = true
          ...
          and nullif(trim(bcm.contract_admin_name), '') is not null
        group by lower(trim(bcm.contract_admin_name))
      ) admins
    ), '[]'::jsonb)
  );
end;
...
declare
  ...
  normalized_contract_admin text := lower(trim(coalesce(p_contract_admin_filter, '')));
begin
          ...
          and (
            ...
            or exists (
              ...
              where bcm.is_operational = true
                and bcm.is_one_to_one = true
                and bcm.contract_id is not null
                and lower(trim(coalesce(bcm.contract_admin_name, ''))) = normalized_contract_admin
...
revoke all on function public.get_hr_roster_bulk_calendar(date, date, text, text, text, text) from public, anon, authenticated;

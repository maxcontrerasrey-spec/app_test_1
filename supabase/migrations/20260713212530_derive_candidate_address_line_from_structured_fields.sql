...
update public.candidate_profiles cp
   set address_line = nullif(
         concat_ws(
           ...
           nullif(trim(coalesce(cp.current_city, '')), '')
         ),
         ''
       )
 where nullif(
...
declare
  ...
  normalized_current_city text := nullif(trim(coalesce(p_current_city, '')), '');
...
begin
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'address_line_source', 'derived_from_street_number_city'
    )
  );
end;
$function$;

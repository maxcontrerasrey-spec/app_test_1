...
-- rollback: forward-only; restore the prior predicate in a subsequent audited migration.
-- purpose: Jornadas must expose and assign only non-private-role workers; historical rows remain intact.
begin;
  ...
  return jsonb_build_object(
    ...
    'workers', coalesce((
      with active_workers as (
        ...
        left join lateral (
          select wr.id as assignment_id, hp.id as pattern_id, hp.name as pattern_name,
            hp.working_days, hp.resting_days, hp.cycle_length,
            wr.start_date as assignment_start_date, wr.end_date as assignment_end_date
          from public.hr_worker_rosters wr join public.hr_shift_patterns hp on hp.id = wr.pattern_id
          ...
          order by wr.start_date desc, wr.created_at desc limit 1
        ) assignment on true
        left join lateral (
        ...
        select wd.buk_employee_id, wd.full_name,
          jsonb_build_object(
            'buk_employee_id', wd.buk_employee_id, 'full_name', wd.full_name,
            'document_number', wd.document_number, 'document_type', wd.document_type,
            'job_title', wd.job_title, 'contract_code', wd.contract_code,
            ...
            'days', jsonb_agg(jsonb_build_object(
              'date', wd.day_date, 'assignment_id', wd.assignment_id, 'pattern_id', wd.pattern_id,
              'pattern_name', wd.pattern_name, 'cycle_day', wd.cycle_day, 'base_status', wd.base_status,
              ...
              'exception_type', wd.exception_type, 'exception_label', wd.exception_label,
              'exception_notes', wd.exception_notes, 'is_working_day', wd.is_working_day,
              'is_rest_day', wd.is_rest_day
            ) order by wd.day_date)
          ) as payload
        from worker_days wd
        group by wd.buk_employee_id, wd.full_name, wd.document_number, wd.document_type,
          wd.job_title, wd.contract_code, wd.area_name, wd.exit_date
      )
...
begin
  ...
  return (
    with active_workers as (
      ...
      inner join filtered_workers fw on fw.buk_employee_id = wr.employee_buk_employee_id
      where wr.start_date <= month_end and coalesce(wr.end_date, 'infinity'::date) >= month_start
    )
...
begin
  ...
  update public.hr_worker_rosters set end_date = p_start_date - 1, updated_at = timezone('utc', now())
  where employee_buk_employee_id = worker_row.buk_employee_id and start_date < p_start_date and coalesce(end_date, 'infinity'::date) >= p_start_date;

  if exists (select 1 from public.hr_worker_rosters wr where wr.employee_buk_employee_id = worker_row.buk_employee_id and daterange(wr.start_date, coalesce(wr.end_date, 'infinity'::date), '[]') && daterange(p_start_date, coalesce(p_end_date, 'infinity'::date), '[]')) then
    raise exception 'Ya existe una asignación de pauta que se superpone con el rango indicado';
  end if;

  insert into public.hr_worker_rosters (employee_buk_employee_id, employee_document_type, employee_document_number, employee_full_name, employee_job_title, contract_code, area_name, pattern_id, start_date, end_date, notes, assigned_by)
  values (worker_row.buk_employee_id, worker_row.document_type, worker_row.document_number, worker_row.full_name, worker_row.job_title, worker_row.contract_code, worker_row.area_name, p_pattern_id, p_start_date, p_end_date, nullif(trim(coalesce(p_notes, '')), ''), current_user_id)

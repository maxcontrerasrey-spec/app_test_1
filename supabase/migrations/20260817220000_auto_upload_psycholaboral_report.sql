...
-- Registers the generated report in the candidate checklist without allowing
-- the automated path to overwrite a manually uploaded document.
...
begin
  ...
  select dt.id into document_type_id
  ...
  select cd.id, cd.file_path, cd.status
   ...
   where cd.recruitment_case_id = assessment_rec.recruitment_case_id
     ...
     and cd.document_type_id = document_type_id
   for update;
  ...
  if existing_rec.id is null then
    insert into public.candidate_documents(
      ...
      document_type_id, 'uploaded', p_file_path, null, timezone('utc', now())
    );
    stored := true;

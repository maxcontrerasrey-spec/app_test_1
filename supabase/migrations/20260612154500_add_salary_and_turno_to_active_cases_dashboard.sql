...
begin
    ...
    from (
      select
        jsonb_build_object(
          ...
          'cost_center_name', rc.cost_center_name,
          'owner_name', owner_profile.full_name,
          'active_process_count', coalesce(candidate_stats.active_process_count, 0),
          ...
          'is_contract_path_blocked', contract_lock.recruitment_case_id is not null,
          'interview_notes', interview_note.note,
          'hired_at', rcc.hired_at
        ) as payload,
        case
          when rc.status = 'ready_to_hire' then 0
          when rc.status = 'screening' then 1
          when rc.status = 'sourcing' then 2
          when rc.status = 'open' then 3
          when rc.status = 'partially_filled' then 4
          when rc.status = 'filled' then 5
          when rc.status = 'closed_unfilled' then 6
          else 7
        end as sort_case_status_priority,
        rc.opened_at as sort_case_opened_at,
        case
          when rcc.stage_code = 'document_review' then 0
          when rcc.stage_code = 'ready_for_hire' then 1
          when rcc.stage_code = 'medical_exams' then 2
          when rcc.stage_code = 'in_process' then 3
          when rcc.stage_code = 'who_approved' then 4
          when rcc.stage_code = 'who_pending' then 5
          when rcc.stage_code = 'screening' then 6
          when rcc.stage_code = 'lead' then 7
          when rcc.stage_code = 'hired' then 8
          else 9
      ...
      left join lateral (
        ...
        from public.recruitment_case_candidates rcc2
        where rcc2.candidate_profile_id = rcc.candidate_profile_id
          and rcc2.stage_code not in ('hired', 'rejected', 'withdrawn')
      ) as candidate_stats on true
      left join lateral public.find_active_candidate_contract_lock(cp.id, rcc.id) as contract_lock on true
      left join lateral (
        select rin.note
        from public.recruitment_interview_notes rin
        where rin.case_candidate_id = rcc.id
        order by rin.created_at desc, rin.id desc
        limit 1
      ) as interview_note on true
      where rcc.stage_code not in ('hired', 'rejected', 'withdrawn')
        and public.user_can_view_recruitment_process_summary(
          current_user_id,
          hr.requester_id,
          hr.cost_center_code
        )
      order by
        sort_case_status_priority asc,
        rc.opened_at desc,
        sort_stage_rank asc,
        rcc.created_at asc
      limit 120
  ...
  if can_access_candidate_control then
    select coalesce(
      jsonb_agg(personnel_row.payload order by personnel_row.sort_case_opened_at desc, personnel_row.sort_ready_rank asc, personnel_row.sort_candidate_created_at asc),
      '[]'::jsonb
    )
    ...
    from (
      select
        jsonb_build_object(
          ...
          'driver_license_expiry', cp.driver_license_expiry,
          'stage_code', rcc.stage_code,
          'stage_entered_at', coalesce(rcc.document_validated_at, rcc.stage_entered_at, rcc.updated_at),
          'suitability_status', rcc.suitability_status,
          'is_selected', rcc.is_selected,
          'contract_name', rc.contract_name,
          'job_position_name', rc.job_position_name,
        ...
        case
          when rcc.stage_code = 'document_review' then 0
          when rcc.stage_code = 'ready_for_hire' then 1
          when rcc.stage_code = 'hired' then 2
          else 3
        end as sort_ready_rank,
        coalesce(rcc.document_validated_at, rcc.stage_entered_at, rcc.updated_at) as sort_candidate_created_at
      from public.recruitment_case_candidates rcc
      join public.candidate_profiles cp
        on cp.id = rcc.candidate_profile_id
      join public.recruitment_cases rc
      ...
      where rcc.stage_code in ('document_review', 'ready_for_hire', 'hired')
        and public.user_can_view_recruitment_process_summary(
          current_user_id,
          hr.requester_id,
          hr.cost_center_code
      ...
      order by
        rc.opened_at desc,
        sort_ready_rank asc,
        coalesce(rcc.document_validated_at, rcc.stage_entered_at, rcc.updated_at) asc
      limit 120
    ) as personnel_row;
  end if;

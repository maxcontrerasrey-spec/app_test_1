...
create index if not exists idx_competency_requests_worker_buk_employee_id
  on public.competency_requests (worker_buk_employee_id);

create index if not exists idx_competency_request_models_model_request
  on public.competency_request_models (model_id, request_id);

create or replace function public.find_competency_certificate_model_conflicts(
  p_worker_buk_employee_id text,
  p_model_ids uuid[],
  p_valid_from date,
  p_valid_until date,
  p_exclude_certificate_id uuid default null
)
returns table (
  certificate_id uuid,
  request_id uuid,
...
as $function$
  ...
  join public.competency_request_models crm
    on crm.request_id = cr.id
  join public.competency_equipment_models m
    on m.id = crm.model_id
  join public.competency_equipment_brands b
    on b.id = m.brand_id
  join public.competency_equipment_types t
    on t.id = m.type_id
  where cr.worker_buk_employee_id = trim(coalesce(p_worker_buk_employee_id, ''))
    and crm.model_id = any(coalesce(p_model_ids, '{}'::uuid[]))
    and (p_exclude_certificate_id is null or cc.id <> p_exclude_certificate_id)
    and cc.competency_status = 'enabled'
    and cc.certificate_status in ('generated', 'uploaded_to_buk')
    and coalesce(cc.valid_from, cr.training_date) <= p_valid_until
    and cc.valid_until >= p_valid_from
  order by cc.valid_until desc, cc.folio asc, b.name asc, t.name asc, m.name asc;
$function$;

revoke all on function public.find_competency_certificate_model_conflicts(text, uuid[], date, date, uuid) from public, anon, authenticated;
grant execute on function public.find_competency_certificate_model_conflicts(text, uuid[], date, date, uuid) to service_role;

create or replace function public.get_competency_certificate_model_warnings(
  p_worker_buk_employee_id text,
  p_model_ids uuid[],
...
declare
  ...
  valid_from_value date := coalesce(p_training_date, current_date);
  valid_until_value date := (coalesce(p_training_date, current_date) + interval '2 years')::date;
begin
  if not public.user_can_access_competencies(current_user_id) then
    raise exception 'Sin permisos para consultar advertencias de certificaciones';
  end if;
  ...
  return jsonb_build_object(
    ...
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'certificateId', conflict.certificate_id,
          'requestId', conflict.request_id,
          'folio', conflict.folio,
          'modelId', conflict.model_id,
          'brandName', conflict.brand_name,
          'typeName', conflict.type_name,
          'modelName', conflict.model_name,
          'validFrom', conflict.valid_from,
          'validUntil', conflict.valid_until
        )
        order by conflict.valid_until desc, conflict.folio asc
      )
      from public.find_competency_certificate_model_conflicts(
        p_worker_buk_employee_id,
        p_model_ids,
        valid_from_value,
        valid_until_value,
        null
...
revoke all on function public.get_competency_certificate_model_warnings(text, uuid[], date) from public, anon;
grant execute on function public.get_competency_certificate_model_warnings(text, uuid[], date) to authenticated;

...
create or replace function public.get_bi_recruitment_dashboard(
  ...
  p_shift_names text[] default null
)
returns jsonb
language plpgsql
stable
...
declare
  ...
  has_full_bi_recruitment_scope boolean := public.user_can_access_feature(auth.uid(), 'bi_reclutamiento');
  ...
  normalized_shifts text[] := public.normalize_bi_text_array(p_shift_names);
  ...
  return (
    with visible_cases as (
      select
        ...
        coalesce(nullif(trim(hr.shift_name), ''), 'SIN JORNADA') as shift_name
      from public.recruitment_cases rc
      join public.hiring_requests hr
        on hr.id = rc.hiring_request_id
      where (
        has_full_bi_recruitment_scope
        or public.user_can_view_hiring_request_process_summary(
          current_user_id,
          hr.requester_id,
          hr.cost_center_code
        )
    ...
    filtered_cases as (
      ...
      where (
        ...
        and (
          coalesce(array_length(normalized_shifts, 1), 0) = 0
          or ac.shift_name = any(normalized_shifts)
        )
    ),
    filtered_all_cases as (
      ...
      where (
        ...
        and (
          coalesce(array_length(normalized_shifts, 1), 0) = 0
          or vc.shift_name = any(normalized_shifts)
        )
    ),
    hired_case_durations as (
      select
        fac.id as recruitment_case_id,
        greatest(
          extract(epoch from (
            min(coalesce(rcc.hired_at, rcc.stage_entered_at, rcc.updated_at, rcc.created_at))
            - fac.opened_at
          )) / 86400.0,
          0
        ) as hiring_days
      ...
      join public.recruitment_case_candidates rcc
        on rcc.recruitment_case_id = fac.id
      where fac.opened_at is not null
        and rcc.stage_code = 'hired'
      group by fac.id, fac.opened_at
      having (
        nullif(trim(coalesce(p_period_code, '')), '') is null
        or min(coalesce(rcc.hired_at, rcc.stage_entered_at, rcc.updated_at, rcc.created_at))::date
    ...
    filtered_mobility as (
      select
        ...
        coalesce(
          nullif(trim(vc.shift_name), ''),
          nullif(trim(imr.destination_shift_name), ''),
          'SIN JORNADA'
        ) as shift_name
      from public.internal_mobility_requests imr
      ...
      where (
          has_full_bi_recruitment_scope
          or public.user_can_view_internal_mobility_request_summary(
            current_user_id,
            imr.requester_id,
    ...
    recruitment_summary as (
      select
        ...
        (
          select round(avg(hcd.hiring_days)::numeric, 1)
          from hired_case_durations hcd
        ) as average_hiring_days
    ...
    shift_options as (
      select distinct ac.shift_name as value
      from active_cases ac
      where (
          coalesce(array_length(normalized_managements, 1), 0) = 0
          or ac.management_name = any(normalized_managements)
    ...
    select jsonb_build_object(
      ...
      jsonb_build_object(
        ...
        'shifts',
        coalesce((
          select jsonb_agg(so.value order by so.value)
      ...
      jsonb_build_object(
        ...
        'averageHiringDays', (select average_hiring_days from recruitment_summary),
...
revoke all on function public.get_bi_recruitment_dashboard(text, text[], text[], text[], text[])
  from public, anon;
grant execute on function public.get_bi_recruitment_dashboard(text, text[], text[], text[], text[])
  to authenticated;
...
revoke all on function public.get_bi_recruitment_daily_timeline(text, text[], text[], text[], text[])
  from public, anon;
grant execute on function public.get_bi_recruitment_daily_timeline(text, text[], text[], text[], text[])
  to authenticated;

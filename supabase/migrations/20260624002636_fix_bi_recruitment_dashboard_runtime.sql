  ...
  return (
    ...
    mobility_summary as (
      select
        ...
        round((
          avg(extract(epoch from (fm.approved_at - fm.submitted_at)) / 3600.0)
            filter (where fm.approved_at is not null)
        )::numeric, 2) as avg_approval_hours,
        round((
          avg(extract(epoch from (fm.hr_execution_executed_at - fm.approved_at)) / 3600.0)
            filter (where fm.hr_execution_executed_at is not null and fm.approved_at is not null)
        )::numeric, 2) as avg_execution_hours
    ...
    approvals_by_step as (
      select
        ...
        round((
          avg(extract(epoch from (fa.decided_at - fa.created_at)) / 3600.0)
            filter (where fa.decided_at is not null)

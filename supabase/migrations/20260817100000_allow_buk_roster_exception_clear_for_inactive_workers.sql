...
begin
  ...
  if normalized_buk_employee_id = '' then
    raise exception 'Debe indicar el trabajador BUK de la excepción';
  end if;
...
revoke all on function public.sync_hr_roster_exception_from_buk(text, date, text, text)
  from public, anon, authenticated;
grant execute on function public.sync_hr_roster_exception_from_buk(text, date, text, text)
  to service_role;

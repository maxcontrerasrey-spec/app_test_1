...
create table if not exists public.operations_contract_editors (
...
alter table public.operations_contract_editors enable row level security;

drop trigger if exists trg_operations_contract_editors_set_updated_at on public.operations_contract_editors;
create trigger trg_operations_contract_editors_set_updated_at
before update on public.operations_contract_editors
for each row execute function public.set_updated_at();

create index if not exists idx_operations_contract_editors_user_active
  on public.operations_contract_editors(user_id, is_active, contract_id);

create index if not exists idx_operations_contract_editors_contract_active
  on public.operations_contract_editors(contract_id, is_active, user_id);

revoke all on public.operations_contract_editors from public, anon;
grant select on public.operations_contract_editors to authenticated;
revoke insert, update, delete on public.operations_contract_editors from authenticated;

drop policy if exists "operations_contract_editors_select_self" on public.operations_contract_editors;
create policy "operations_contract_editors_select_self"
on public.operations_contract_editors
for select
to authenticated
using (
...
with target_contract as (
  ...
  where c.is_active = true
    and (c.code = 'CONT-028' or c.contract_name in ('CODELCO DMH', 'SERVICIO CODELCO DMH'))
  order by case when c.code = 'CONT-028' then 0 else 1 end, c.id
  limit 1
), target_users as (
  select p.id as user_id, lower(p.email) as email
  from public.profiles p
  where p.status = 'active'
    and lower(p.email) in ('jose.orellana@busesjm.com', 'supervisor.dmh@busesjm.com')
)
insert into public.operations_contract_editors (user_id, contract_id, is_active, reason)
select target_users.user_id, target_contract.contract_id, true, 'Editor inicial CODELCO DMH Operaciones'
from target_users
...
begin
  if (
    ...
    join public.contracts c
      on c.id = oce.contract_id
    where lower(p.email) in ('jose.orellana@busesjm.com', 'supervisor.dmh@busesjm.com')
      and c.code = 'CONT-028'
      and oce.is_active = true
  ) <> 2 then
    raise exception 'CODELCO DMH debe quedar editable para Jose Orellana y supervisor.dmh.';
  end if;
end
$$;
...
begin
  ...
  loop
    ...
    if not public.user_can_edit_operations_contract(current_user_id, resolved_contract_id) then
      errors := errors || jsonb_build_object(
        'index', entry_index - 1,
        'service_id', entry_item->>'serviceId',
        'field_errors', jsonb_build_object('contractCode', 'No tienes permiso para editar servicios de este contrato.')
      );
      continue;
    end if;

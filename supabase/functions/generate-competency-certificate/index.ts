...
function drawModelSummary(
  ...
  const rowLayouts = visibleRows.map((row) => {
    const cells = [
      buildWrappedCellTextLayout(row.brand_name, fonts.regular, rowFontSize, columns[0] - 8),
      buildWrappedCellTextLayout(row.type_name, fonts.regular, rowFontSize, columns[1] - 8),
      buildWrappedCellTextLayout(row.model_name, fonts.regular, rowFontSize, columns[2] - 8)
    ];
    return {
      cells,

...
- [x] Resumen Ejecutivo: "Puntos de Atención" (texto decorativo, 2 hechos posibles) reemplazado por "Focos de Gestión" — 3 hechos comparativos calculados 100% dinámicamente (mayor ausentismo hoy, mayor brecha de contratación, mayor monto de incentivos), cada uno omitido si la sección no está autorizada por permisos o si no hay datos con valor > 0. Sin thresholds ni calificativos inventados.
- [x] Corregido bug real: "Top contratos con mayor ausentismo (hoy)" (Resumen y bloque Dotación y Asistencia) mostraba `contract_code` crudo (`get_bi_presence_summary_today` no devuelve `area_name`). Resuelto reutilizando el mismo mapping `contractCode -> areaName` (via `get_bi_headcount_by_contract`, ya cacheado) y el mismo helper `buildAbsenteeismByContractRows` que ya usaba `BiAbsenteeismByContractChart` en Dotación — sin duplicar lógica, sin tocar Supabase. Se agregó también el desglose "N ausentes / N personas" pedido.
- [x] Reclutamiento: eliminado el gráfico grande "Cobertura de Vacantes" (redundante con el KPI "Cobertura" superior, que se mantiene). "Brecha de Contratación por Contrato" queda como único visual principal de esa sección, ya ordenado por vacantes pendientes DESC.
- [x] Reclutamiento: eliminado "Pulso Operativo" como concepto (mezclaba folios/vacantes/cobertura acumulada de forma poco clara). Reemplazado por "Ritmo de Incorporación": columnas apiladas de personas realmente incorporadas (Contratados=azul + Movilidad interna ejecutada=verde) por período, con selector D/S/M/6M reutilizando la misma agregación temporal ya validada (exclusión de buckets futuros vía `isFutureBucketDate`, sin ceros artificiales) y tooltip con desglose + total.
- [x] Antigüedad de Vacantes: NO implementada (mantenida fuera de alcance frontend-only). Ningún RPC actual (`get_bi_recruitment_dashboard`, `get_bi_recruitment_pipeline`) expone fecha de apertura por vacante/folio individual — solo conteos agregados. Requeriría ampliar backend para poder calcularse correctamente; queda documentado, no inventado.
- [x] Verificado visualmente con datos simulados (arnés temporal con `AuthContext` mockeado vía `export` temporal + `git checkout` para revertir, ya eliminado junto con `.claude/launch.json`; sin credenciales reales de Supabase en este entorno): Resumen Ejecutivo (Focos de Gestión con 3 hechos correctos, nombres de contrato correctos en el ranking de ausentismo) y Reclutamiento (sin Cobertura de Vacantes, sin Pulso Operativo, Ritmo de Incorporación con tabs D/S/M/6M) renderizando correctamente. Dotación (no tocada) se verificó intacta.
- [x] `tsc -b`, `vite build`, `git diff --check`, `npm run guardian`: todo pasa salvo `audit:performance-baseline` (crecimiento esperado por el código nuevo; no se actualizó el baseline, queda documentado).

Alcance explícito: no se modificó Supabase/migraciones/RLS/permisos. No se tocó Dotación BUK (aparte de reutilizar su helper de labels) ni Incentivos (aparte de reutilizar sus datos ya cacheados en el bloque de Resumen). No se hizo commit ni push.

## Rediseño BI - Analítica de Dotación (BUK) - 2026-08-17

- [x] Auditar implementación actual de Dotación (componentes, hooks, RPCs) y detectar bugs reales antes de rediseñar: `BiRecruitmentFunnel` usaba stage_codes inventados que nunca coincidían con los reales (100% de candidatos caía en "Aplicantes"); `BiTrendingExceptionsChart` tomaba `absenteeismPct` de una fila arbitraria vía `.find()` en vez de combinar correctamente entre contratos; `get_bi_exceptions_monthly` solo soporta un período a la vez (no existe tendencia multi-mes real en el backend actual).
- [x] Dotación por Contrato: pie/rose → barras horizontales, orden headcount desc, dataZoom, tooltip con % del total.
- [x] Dotación por Ciudad/mapa: eliminado del dashboard (RPC/hook intactos, sin borrar funcionalidad de datos). Reemplazado por Dotación por Cargo tras no poder verificar en vivo la calidad de `job_title` (sin acceso a Supabase en este entorno); implementado de forma honesta (sin taxonomía inventada) y documentado el riesgo.
- [x] Presencia General: gauge → anillo radial (ring) con % protagonista al centro, sin thresholds de color inventados (solo verde=presencia/gris=resto).
- [x] Composición de Ausentismo: donut rediseñado con semántica de color consistente (vacaciones=azul, licencia médica=rojo, otras=naranjo) y total al centro.
- [x] Distribución Etaria: retirada del dashboard (RPC/hook intactos). Reemplazada por Ausentismo por Contrato (100-presence_pct por contrato, `get_bi_presence_summary_today`), sin calificar "crítico/bueno/malo".
- [x] Candidatos/Etapas: funnel → "Candidatos por Etapa" en orden operacional real, corrigiendo el bug de stage_codes y excluyendo estados terminales (hired/rejected/withdrawn).
- [x] Tendencia de Ausentismo: reconstruida como tendencia mensual real (6 meses) orquestando 6 llamadas paralelas a `get_bi_exceptions_monthly` (mismo patrón que `fetchBiRecruitmentDashboard`/Incentivos), columnas apiladas + línea de ausentismo % derivada correctamente de `fteHeadcountEquivalent`/`headcountBase` sumados entre contratos (no de una fila arbitraria).
- [x] Verificar visualmente con datos simulados (sin acceso a Supabase real en este entorno): las 5 visualizaciones de barras/línea (Dotación por Contrato, Dotación por Cargo, Ausentismo por Contrato, Candidatos por Etapa, Evolución Mensual) confirmadas renderizando correctamente con números reales, orden y colores esperados. Los 2 gráficos tipo pie/anillo (Presencia General, Composición de Ausentismo) no se pudieron confirmar visualmente en este entorno: la herramienta de navegador automatizado no rellena los arcos del pie (solo dibuja las líneas guía de etiqueta), mientras que rectángulos/líneas sí renderizan; se aisló con una prueba mínima sin wrapper ni estilos custom y persistió igual en dev y en build de producción (`vite preview`). Misma configuración que el donut de Incentivos ya en producción (sesión anterior), así que el código sigue el patrón establecido, pero el relleno de los pie charts queda pendiente de confirmación visual directa por el usuario.
- [x] `tsc -b`, `vite build`, `git diff --check`, `npm run guardian`: todo pasa salvo `audit:performance-baseline` (dist +17.6KB / JS +11.9KB / CSS +5.7KB vs baseline, esperado por la nueva lógica; NO se actualizó el baseline, queda documentado para decisión del usuario).

Alcance explícito: no se modificó Supabase/migraciones/RLS/permisos. No se hizo commit ni push.

## Rediseño BI (Reclutamiento, Incentivos) y nueva vista Resumen Ejecutivo - 2026-08-17

- [x] Auditar contrato real de backend BI (RPCs `get_bi_recruitment_dashboard`, `get_bi_recruitment_daily_timeline`, `get_bi_workforce_overview`, `get_bi_presence_summary_today`, `get_hr_incentives_analytics`) y documentar qué dato soporta cada KPI antes de tocar componentes.
- [x] Reclutamiento: reordenar KPIs superiores (Folios, Vacantes pendientes, Cobertura %, Candidatos activos, Tiempo medio), reemplazar donut de cobertura por bullet/progress bar, convertir "Cupos por Contrato" en "Brecha de Contratación por Contrato" (barras horizontales, orden por faltantes desc), validar si Etapas de Candidatos es un pipeline estrictamente secuencial (spoiler: no, `Levantamiento de Contraindicación` es rama opcional) y decidir funnel vs barras en consecuencia, reemplazar donut de Movilidad Interna por barras horizontales conservando semántica de color ya existente, corregir Pulso Operativo para no mostrar fechas futuras como cero (usar fecha cliente vs `bucketStart`) y rediseñar la serie (folios abiertos + cubiertos + cobertura acumulada %).
- [x] Incentivos: validar semántica real de "Gasto total" contra el RPC (incluye todos los estados, no solo aprobados) y renombrar a "Monto registrado"; agregar Monto aprobado/pendiente derivados de `amount_by_period` ya entregado (sin RPC nuevo); quitar redundancia barra+línea en evolución; agregar total al centro del donut de distribución por tipo; quitar gradientes de inversión por contrato y agregar %; simplificar ranking de trabajadores a una sola barra por monto total con contratos en tooltip; agregar visualización compacta de estado de solicitudes (aprobadas/pendientes/rechazadas).
- [x] Crear cuarta vista "Resumen Ejecutivo" (`/bi/resumen`) reutilizando hooks/queries existentes (sin duplicar cálculos), con bloques compactos de Dotación, Reclutamiento e Incentivos, cada uno con "Ver detalle" via React Router, respetando `bi_dotacion`/`bi_incentivos`/`bi_reclutamiento` por bloque. Sin nuevo feature backend: la pestaña se gatilla por el módulo `bi_analytics` ya existente. Cambiar default de `/bi` a `/bi/resumen`.
- [x] Validar visualmente en localhost las 4 vistas (arnés temporal con AuthContext mockeado, ya revertido; sin credenciales reales de Supabase en este entorno), ejecutar `tsc -b`, `vite build`, `git diff --check` y `npm run guardian`. NO commit/push (pedido explícito del usuario).

Alcance explícito: no se modifica Supabase/migraciones/RLS/permisos en esta entrega; cualquier KPI que requiera nuevo dato backend queda documentado como pendiente, no inventado.

Resultado: las 4 vistas de BI (`resumen`, `dotacion`, `incentivos`, `reclutamiento`) están implementadas y verificadas visualmente con datos simulados (sin acceso a Supabase real en este entorno). `tsc -b`, `vite build` y `git diff --check` pasan limpios. `npm run guardian` da 1 error esperado: `audit:performance-baseline` (dist +8KB, JS +3KB, CSS +5KB por las vistas nuevas), pendiente de actualizar `eees/baselines/PERFORMANCE_BASELINE_v1.md` junto con el commit real. Durante la verificación se corrigió un bug real descubierto en pruebas: los 3 bloques del Resumen Ejecutivo mostraban "Cargando..." indefinidamente ante un error de query en vez de un estado de error distinto (ya corregido, sigue el patrón existente de `BiOverviewCards.tsx`). No se hizo commit ni push.

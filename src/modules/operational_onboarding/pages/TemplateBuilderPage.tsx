      ...
      <div className="departments-board">
        {tasksLoading ? (
          ...
          <div className="empty-lane">
            <p>Aún no hay áreas ni tareas configuradas. Comienza agregando un área.</p>
          </div>
        ) : (
          Object.entries(tasksByArea).map(([area, areaTasks]) => (
            <div key={area} className="department-lane">
              <div className="department-header">
                <h3>{area} <span className="text-gray-400 font-normal ml-2">({areaTasks.length})</span></h3>
                <button className="btn-secondary text-sm py-1" onClick={() => handleAddTask(area)}>
                  + Añadir Tarea
                </button>
              </div>
                  ...
                  <div key={task.id} className="task-card">
                    <div className="task-card-header">
                      ...
                      <div className="task-actions">
                        <button className="icon-btn" onClick={() => alert("Modal de Edición próximamente")}>✏️</button>
                        <button className="icon-btn" onClick={() => handleDeleteTask(task.id)}>🗑️</button>
                      </div>
                    </div>
                    {task.task_description && (
                      <p className="text-sm text-gray-500 line-clamp-2 m-0">{task.task_description}</p>
                    )}
                    <div className="task-badges">
                      {task.is_blocking && <span className="task-badge badge-blocking">Bloqueante</span>}
                      {task.requires_evidence && <span className="task-badge badge-evidence">Evidencia: {task.evidence_type || 'Archivo'}</span>}
                      {task.sla_hours && <span className="task-badge badge-sla">SLA: {task.sla_hours}h</span>}
                    </div>
                  </div>
                ))}
              </div>
